...
object Tests extends Suite(m"Consequent Tests"):
  ...
  def stub(body: String): String =
    ...
    builder.append("*/\n")
    builder.append("package consequent\n")
    builder.append("\n")
    builder.append(body)
    builder.toString

  def violations(body: String): List[Violation] =
    Checker.check("<test>", Some("consequent"), stub(body), config = config).toList
  ...
  def run(): Unit =
    ...
    suite(m"Phase 1: License, package, imports"):
      ...
      test(m"Missing blank after package is rejected"):
        ...
        Checker.check("<test>", Some("consequent"), builder.toString).toList.map(_.rule)
