...
// D1 — the "necessity" extractor (Principle P3, Density): the most
...
// the line limit is a violation — a break must always mean "didn't fit".
//
...
object Necessity:
  ...
  // emission point for D1); `width` is the column count of the one-line
  ...
  def extract
    ( tree: untpd.Tree, source: SourceFile, text: String, columns: Int = 100 )
  :   List[Site] =

    val out = mutable.ListBuffer[Site]()

    def offer(node: untpd.Tree, kind: String): Unit =
      siteFor(node, kind, source, text, columns).foreach(out += _)
    ...
    // without breaking the body-cascade rule (A2.2) — so only the top of
  ...
  // one-lineable, and its collapsed rendering fits within `columns` columns.
  private def siteFor
    ( node: untpd.Tree, kind: String, source: SourceFile, text: String, columns: Int )
  :   Option[Site] =
        ...
        if text.substring(lineStart, suffixEnd).nn.contains('\f') then None
          ...
          collapsedText(text, start, end, colonException, isLambda).flatMap: collapsed =>
            ...
            if width <= columns then
              ...
              // be *rewritten* brace-wrapped on one line (D2.1) — the same
  ...
  // (`{ λ }`) on one line per D2.1, which is two columns wider than the
